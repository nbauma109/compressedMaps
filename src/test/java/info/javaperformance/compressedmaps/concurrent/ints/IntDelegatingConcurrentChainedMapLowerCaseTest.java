...
public class IntDelegatingConcurrentChainedMapLowerCaseTest extends TestCase {
	...
	private void testPutHelper(final int threads, final float ff) throws InterruptedException {
		...
		// initial insertion section
		{
			...
			for (int n = 0; n < SECTION * threads; ++n) {
				assertEquals(Integer.toString(n, 36), map.get(n));
			}
		}
		// update section
		{
		...
		public void run() {
			try {
				...
				for (int n = m_from; n < m_to; ++n) {
					assertEquals(NOT_PRESENT, m_map.put(n, Integer.toString(n, 36)));
					assertEquals(Integer.toString(n, 36), m_map.get(n));
				}
			} catch (Throwable e) {
				e.printStackTrace();
			}
	...
	private static class Updater implements Runnable {
		...
		public void run() {
			try {
				...
				m_startGate.await();
				for (int n = m_from; n < m_to; ++n) {
					assertEquals(Integer.toString(n, 36), m_map.put(n, ONE));
