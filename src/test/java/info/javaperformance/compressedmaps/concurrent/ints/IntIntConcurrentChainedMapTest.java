...
public class IntIntConcurrentChainedMapTest extends TestCase {
	...
	protected IIntIntConcurrentMap getMap(final int size, final float ff) {
		return IntMapFactory.concurrentIntIntMap(size, ff);
	}
	...
	private void testPutHelper(final int threads, final float ff) throws InterruptedException {
		...
		final IIntIntConcurrentMap map = getMap(INITIAL_CAPACITY, ff);
		...
		// initial insertion section
		{
			...
			end.await(100, TimeUnit.SECONDS); // more than enough, needed if one
												// of threads dies
	...
	private static class Adder implements Runnable {
		...
		private final IIntIntConcurrentMap m_map;

		public Adder(int from, int to, CountDownLatch startGate, CountDownLatch endGate, IIntIntConcurrentMap map) {
			m_from = from;
			m_to = to;
		...
		public void run() {
			try {
				...
				for (int n = m_from; n < m_to; ++n) {
					assertEquals(NOT_PRESENT, m_map.put(n, n));
					assertEquals(n, m_map.get(n));
				}
			} catch (Throwable e) {
				e.printStackTrace();
			}
	...
	private static class Updater implements Runnable {
		...
		public Updater(int from, int to, CountDownLatch startGate, CountDownLatch endGate, IIntIntConcurrentMap map) {
			m_from = from;
			m_to = to;
			m_startGate = startGate;
			m_endGate = endGate;
		...
		public void run() {
			try {
				...
				m_startGate.await();
				for (int n = m_from; n < m_to; ++n) {
					assertEquals(n, m_map.put(n, ONE));
	...
	/*
	 * Thread N adds a set of entries, updates them and removes half of its own
	 * values. We can not remove other thread entries because they may not be
	 * present in the map at that time.
	 */
	...
	private static class AddUpdateRemover implements Runnable {
		...
		public AddUpdateRemover(int from, int to, CountDownLatch startGate, CountDownLatch endGate, IIntIntConcurrentMap map) {
			m_from = from;
			m_to = to;
			m_startGate = startGate;
			m_endGate = endGate;
	...
	private static class Remover implements Runnable {
		...
		public Remover(int from, int to, CountDownLatch startGate, CountDownLatch endGate, IIntIntConcurrentMap map) {
			m_from = from;
			m_to = to;
			m_startGate = startGate;
			m_endGate = endGate;
