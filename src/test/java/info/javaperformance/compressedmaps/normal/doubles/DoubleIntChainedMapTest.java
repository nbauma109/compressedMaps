...
public class DoubleIntChainedMapTest extends TestCase
{
    ...
    protected IDoubleIntMap makeMap( final long size, final float fillFactor )
    {
        return DoubleMapFactory.singleThreadedDoubleIntMap( size, fillFactor );
    }
    ...
    {
        final IDoubleIntMap map = makeMap(100, fillFactor);
    ...
    private void testRandomRemoveHelper( final float ff )
    {
        ...
        IDoubleIntMap m = makeMap( 100, ff );
        int add = 0, remove = 0;
        while ( add < SIZE )
        {
