...
public class DoubleDoubleChainedMapTest extends TestCase
{
    ...
    protected IDoubleDoubleMap makeMap( final long size, final float fillFactor )
    {
        return DoubleMapFactory.singleThreadedDoubleDoubleMap( size, fillFactor );
    }
    ...
    {
        final IDoubleDoubleMap map = makeMap(100, fillFactor);
    ...
    private void testRandomRemoveHelper( final float ff )
    {
        ...
        IDoubleDoubleMap m = makeMap( 100, ff );
        int add = 0, remove = 0;
        while ( add < SIZE )
        {
