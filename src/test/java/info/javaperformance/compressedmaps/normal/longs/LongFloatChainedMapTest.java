...
public class LongFloatChainedMapTest extends TestCase
{
    ...
    protected ILongFloatMap makeMap( final long size, final float fillFactor )
    {
        return LongMapFactory.singleThreadedLongFloatMap( size, fillFactor );
    }
    ...
    {
        final ILongFloatMap map = makeMap(100, fillFactor);
    ...
    private void testRandomRemoveHelper( final float ff )
    {
        ...
        ILongFloatMap m = makeMap( 100, ff );
        int add = 0, remove = 0;
        while ( add < SIZE )
        {
