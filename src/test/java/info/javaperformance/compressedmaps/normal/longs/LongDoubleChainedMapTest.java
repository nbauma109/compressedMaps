...
public class LongDoubleChainedMapTest extends TestCase
{
    ...
    protected ILongDoubleMap makeMap( final long size, final float fillFactor )
    {
        return LongMapFactory.singleThreadedLongDoubleMap( size, fillFactor );
    }
    ...
    {
        final ILongDoubleMap map = makeMap(100, fillFactor);
    ...
    private void testPutThenUpdate( final float fillFactor )
    {
        ...
        for ( int i = 0; i < SIZE; ++i )
        {
            assertEquals( ( double )i, map.put( i, i + 1 ) );
            assertEquals( SIZE, map.size() );
    ...
    private void testPutRandom( final float fillFactor )
    {
        ...
        {
            assertEquals( NOT_PRESENT, map.put( vals[i], (double)vals[i]  ) );
            assertEquals( i + 1, map.size());
    ...
    private void testRemoveHelper( final float fillFactor )
    {
        ...
        for ( int i = 0; i < SIZE; ++i )
        {
            ...
            assertEquals( (double)removeCnt, map.remove(removeCnt));
            removeCnt++;
    ...
    private void testRandomRemoveHelper( final float ff )
    {
        ...
        ILongDoubleMap m = makeMap( 100, ff );
        int add = 0, remove = 0;
        while ( add < SIZE )
        {
