...
public class IntBigIntegerChainedMapTest extends TestCase {
	...
	private void testPutHelper(final float fillFactor) {
		final IIntObjectMap<BigInteger> map = makeMap(100, fillFactor);
		for (int i = 0; i < SIZE; ++i) {
			assertEquals(NOT_PRESENT, map.put(i, BigInteger.valueOf(i)));

			assertEquals(i + 1, map.size());
			assertEquals(BigInteger.valueOf(i), map.get(i));
		}
	...
	private void testPutNegative(final float fillFactor) {
		final IIntObjectMap<BigInteger> map = makeMap(100, fillFactor);
		for (int i = 0; i < SIZE; ++i) {
			map.put(-i, BigInteger.valueOf(-i));
			assertEquals(i + 1, map.size());
			assertEquals(BigInteger.valueOf(-i), map.get(-i));
		}
		// now check the final state
	...
	private void testPutThenUpdate(final float fillFactor) {
		...
		for (int i = 0; i < SIZE; ++i) {
			assertEquals(BigInteger.valueOf(i), map.put(i, BigInteger.valueOf(i + 1)));
			assertEquals(SIZE, map.size());
			assertEquals(BigInteger.valueOf(i + 1), map.get(i));
		}
	...
	private void testPutRandom(final float fillFactor) {
		...
		for (i = 0; i < vals.length; ++i) {
			assertEquals(NOT_PRESENT, map.put(vals[i], BigInteger.valueOf(vals[i])));
			assertEquals(i + 1, map.size());
			assertEquals(BigInteger.valueOf(vals[i]), map.get(vals[i]));
		}
		// now check the final state
		assertEquals(SIZE, map.size());
		for (i = 0; i < vals.length; ++i) {
	...
	private void testRemoveHelper(final float fillFactor) {
		...
		for (int i = 0; i < SIZE; ++i) {
			assertEquals(NOT_PRESENT, map.put(addCnt, BigInteger.valueOf(addCnt)));
			assertEquals(i + 1, map.size());
			addCnt++;
			...
			assertEquals(BigInteger.valueOf(removeCnt), map.remove(removeCnt));
			removeCnt++;
	...
	private void testRandomRemoveHelper(final float ff) {
		...
		final BigInteger[] values = new BigInteger[SIZE];
		...
		for (int i = 0; i < SIZE; ++i) {
			values[i] = BigInteger.valueOf(r.nextInt());
		}

		IIntObjectMap<BigInteger> m = makeMap(100, ff);
		int add = 0, remove = 0;
		while (add < SIZE) {
			assertEquals(NOT_PRESENT, m.put(keys[add], new BigInteger(values[add].toString())));
			++add;
			assertEquals(NOT_PRESENT, m.put(keys[add], new BigInteger(values[add].toString())));
			++add;

			assertEquals(new BigInteger(values[remove].toString()), m.remove(keys[remove]));
			remove++;
		...
		for (int i = SIZE / 2; i < SIZE; ++i) {
			assertEquals(new BigInteger(values[i].toString()), m.get(keys[i]));
		}
	}
