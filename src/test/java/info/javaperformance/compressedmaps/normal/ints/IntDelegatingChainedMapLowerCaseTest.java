...
public class IntDelegatingChainedMapLowerCaseTest extends TestCase {
	...
	private void testPutRandomLong(final float fillFactor) {
		...
		final IIntObjectMap<String> map = makeMap(100, fillFactor);
		for (i = 0; i < vals.length; ++i) {
			assertEquals(NOT_PRESENT, map.put(keys[i], Long.toString(vals[i], 36).toLowerCase()));
			assertEquals(i + 1, map.size());
			assertEquals(Long.toString(vals[i], 36).toLowerCase(), map.get(keys[i]));
		}
		// now check the final state
		assertEquals(SIZE, map.size());
		for (i = 0; i < vals.length; ++i) {
