...
public class FloatFloatChainedMapTest extends TestCase
{
    ...
    protected IFloatFloatMap makeMap( final long size, final float fillFactor )
    {
        return FloatMapFactory.singleThreadedFloatFloatMap( size, fillFactor );
    }
    ...
    {
        final IFloatFloatMap map = makeMap(100, fillFactor);
    ...
    private void testRandomRemoveHelper( final float ff )
    {
        ...
        IFloatFloatMap m = makeMap( 100, ff );
        int add = 0, remove = 0;
        while ( add < SIZE )
        {
