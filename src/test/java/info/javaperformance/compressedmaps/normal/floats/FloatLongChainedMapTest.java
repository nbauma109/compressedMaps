...
public class FloatLongChainedMapTest extends TestCase
{
    ...
    protected IFloatLongMap makeMap( final long size, final float fillFactor )
    {
        return FloatMapFactory.singleThreadedFloatLongMap( size, fillFactor );
    }
    ...
    {
        final IFloatLongMap map = makeMap(100, fillFactor);
    ...
    private void testRandomRemoveHelper( final float ff )
    {
        ...
        IFloatLongMap m = makeMap( 100, ff );
        int add = 0, remove = 0;
        while ( add < SIZE )
        {
