...
/**
 * This class is derived from Java 8 ConcurrentHashMap. The main differences are
 * lack of TreeMap support and replacement of Object keys with {@code int} keys.
 *
 * This class will be replaced with another implementation in one of subsequent
 * releases.
 *
 * @param <V>
 *            Value type
...
public class ConcurrentIntObjectMap<V> {
	...
	static class Node<V> {
		...
		public final boolean equals(Object o) {
			...
			Object v, u;
			return (v = e.getValue()) != null && e.getKey() == key && (v == (u = val) || v.equals(u));
		}
	...
	static <K, V> boolean casTabAt(Node<V>[] tab, int i, Node<V> c, Node<V> v) {
	...
	private void addCount(long x, int check) {
		...
		if ((as = counterCells) != null || !U.compareAndSwapLong(this, BASECOUNT, b = baseCount, s = b + x)) {
			...
			if (as == null || (m = as.length - 1) < 0 || (a = as[ThreadLocalRandom.current().nextInt() & m]) == null
	...
	static {
		try {
			...
			Class<?> k = ConcurrentIntObjectMap.class;
