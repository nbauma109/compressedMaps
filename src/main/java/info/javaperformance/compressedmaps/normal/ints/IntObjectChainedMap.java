...
public class IntObjectChainedMap<V> implements IIntObjectMap<V> {
	 ...
	 */
	public IntObjectChainedMap(final long size, final float fillFactor, final IIntSerializer keySerializer, final IObjectSerializer<V> valueSerializer,
	...
	private UpdateResult<V> addToChainRehash(final int index, final Iterator<V> inputIter) {
		...
		final Iterator<V> iter = m_iter.reset(input, m_data);
		if (iter.getElems() > m_data.maxEncodedLength() - 2) {
			return addToChainSlow(index, iter, inputBlock, inputStartOffset, inputIter.getKey(), inputIter.readValue()); // read
																															// a
																															// value
																															// for
																															// less
	 ...
	 * @return A new chain and an old value
	 */
	private UpdateResult<V> addToChainSlow(final int index, final Iterator<V> iter, final SingleThreadedBlock inputBlock, final int inputStartOffset, final int key,
			final V value) {
		...
		final SingleThreadedBlock outputBlock = getBlock(chainLength + 2 * m_keySerializer.getMaxLength() + m_valueSerializer.getMaxLength(value) + 2); // 2
																																						// for
																																						// transition
																																						// from
																																						// header
	...
	private static class Iterator<V> {
		...
		/**
		 * Get value length if the iterator is currently standing prior to the
		 * value (using advance(false) ). This method does not change the input
		 * ByteArray / iterator state.
		 *
