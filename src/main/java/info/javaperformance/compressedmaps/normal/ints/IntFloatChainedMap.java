...
/**
 * A simple single threaded compressed map. It uses {@code int[]} instead of
 * {@code long[]} for buckets before it allocates 128K blocks (or chain length
 * exceeds 4K), so that the smaller maps can benefit from the even smaller map
 * footprint.
 */
public class IntFloatChainedMap implements IIntFloatMap {
	...
	/**
	 * Map size. We are not limited by int because of possibility to have fill
	 * factors greater than 1.
	 */
	private long m_size;

	/**
	 * Map size threshold - next rehashing happens after we exceed the
	 * threshold. This field is {@code long} because we can use fill factors >
	 * 1.
	 */
	 ...
	 *            up with worse memory consumption than {@code HashMap} can
	 *            provide you.
	 * @param keySerializer
	 *            Serializer for keys
	 * @param valueSerializer
	 *            Serializer for values
	 * @param blockCacheLimit
	 *            The limit on the amount of memory blocks we try to reuse in
	 *            order to reduce the GC load. Increase it over the default
	 *            (32K) if you want nearly no GC impact after the map size will
	 *            stabilize.
	 *
	 ...
	 *             If {@code fillFactor > 16} or {@code fillFactor <= 0.01} or
	 *             {@code blockCacheLimit < 0}
	 */
	public IntFloatChainedMap(final long size, final float fillFactor, final IIntSerializer keySerializer, final IFloatSerializer valueSerializer, final long blockCacheLimit) {
	 ...
	 *            Bucket to use
	 */
	private void singleEntry(final SingleThreadedBlock output, final int key, final float value, final int idx) {
	 ...
	 */
	private UpdateResult addToChain(final int index, final int key, final float value) {
		...
		if (iter.getElems() > m_data.maxEncodedLength() - 2) {
		...
		inputBlock.decreaseEntries(); // release the input block, it may be held
										// by this method for a little longer
	 ...
	 * @return A new chain and an old value
	 */
	private UpdateResult addToChainSlow(final int index, final Iterator iter, final SingleThreadedBlock inputBlock, final int inputStartOffset, final int key, final float value) {
		...
		final SingleThreadedBlock outputBlock = getBlock(2 * m_singleEntryLength + chainLength + 2); // 2
																										// for
																										// transition
																										// from
																										// header
	...
	/**
	 * Remove a given key from a chain. 3 special cases are supported: 1) key
	...
	private UpdateResult removeKey(final int key, final int idx) {
		...
		// special case 2 - removal of the last element and length is not too
		// big - only bucket info changes
		// we can not check for equality - once we have reached maxlen there
		// will be a length field at the start of the record
		if (!iter.hasNext() && iter.getElems() < m_data.maxEncodedLength()) {
			m_data.set(idx, inputBlock.getIndex(), inputStartOffset, iter.getElems() - 1);
			return m_updateResult.set(retValue, -1);
		}

		// Reuse the same block for removed chain. There are some risks that it
		// will keep more blocks active, but on
		// the other hand it will definitely reduce the number of discarded
		// blocks => reduce GC.
		...
		m_data.set(idx, inputBlock.getIndex(), inputStartOffset, iter.getElems() <= m_data.maxEncodedLength() ? iter.getElems() - 1 : m_data.maxEncodedLength());
		return m_updateResult.set(retValue, -1);
	}

	/**
	 * Get map size. Note that this method does not lock the map to calculate
	...
	private void rehashInnerStep(final Buckets old, final ByteArray bar, final Iterator iter) {
		...
		if (old.getBlockLength() == 1) // shortcut, no data copy for blocklen =
										// 1
		{
			iter.advance();
	...
	private static class Iterator {
		 ...
		 *            Byte buffer
		 * @param data
		 *            A reference to a current Buckets object
	...
	/**
	 * All updating methods generate this structure containing a new chain
	 * information
	 */
	private static class UpdateResult {
		...
		public UpdateResult set(float retValue, int sizeChange) {
		...
		public String toString() {
			return "UpdateResult{" + "retValue=" + retValue + ", sizeChange=" + sizeChange + '}';
		}
	}
