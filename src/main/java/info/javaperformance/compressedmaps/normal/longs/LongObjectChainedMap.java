 ...
 */
public class LongObjectChainedMap<V> implements ILongObjectMap<V>{
     ...
     */
    public LongObjectChainedMap( final long size, final float fillFactor,
     ...
     */
    private void singleEntry( final SingleThreadedBlock output, final long key, final V value, final int idx )
    {
     ...
     */
    private UpdateResult<V> addToChain( final int index, final long key, final V value )
    {
     ...
     */
    private UpdateResult<V> removeKey( final long key, final int idx )
    {
        final SingleThreadedBlock inputBlock = getBlockByIndex( m_data.getBlockIndex() );
        final int inputStartOffset = m_data.getOffset();
