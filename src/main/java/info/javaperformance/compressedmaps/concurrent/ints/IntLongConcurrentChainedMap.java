...
public class IntLongConcurrentChainedMap implements IIntLongConcurrentMap {
	 ...
	 */
	public IntLongConcurrentChainedMap(final long size, final float fillFactor, final IIntSerializer keySerializer, final ILongSerializer valueSerializer) {
	...
	public long put(final int key, final long value) {
		...
		while (true) {
			...
			if (res != null && compareAndSet(buffers.cur, idx, bucket, res.chain)) {
				...
				final long ret = res.retValue; // must be saved in case of
												// rehash
	 ...
	 */
	private void doPutRehash(final long[] tab, final int key, final long value) {
		final int idx = getIndex(key, tab.length);
	 ...
	 */
	private long singleEntry(final Block output, final int key, final long value) {
	 ...
	 */
	private UpdateResult addToChain(final long bucket, final int key, final long value) {
		if (bucket == EMPTY) {
	 ...
	 * @return A new chain and an old value
	 */
	private UpdateResult addToChainSlow(final long bucket, final Iterator iter, final Block inputBlock, final int inputStartOffset, final int key, final long value) {
	...
	private static class Iterator {
		 ...
		 */
		public long findKey(final int key, final long noValue) {
			while (hasNext()) {
				advance();
				if (getKey() == key) {
					return getValue();
