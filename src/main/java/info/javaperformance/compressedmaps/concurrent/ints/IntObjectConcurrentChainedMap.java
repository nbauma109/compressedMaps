...
public class IntObjectConcurrentChainedMap<V> implements IIntObjectConcurrentMap<V> {
	 ...
	 */
	public IntObjectConcurrentChainedMap(final long size, final float fillFactor, final IIntSerializer keySerializer, final IObjectSerializer<V> valueSerializer) {
	 ...
	 */
	private void doPutRehash(final long[] tab, final int key, final V value) {
		final int idx = getIndex(key, tab.length);
	 ...
	 */
	private long singleEntry(final Block output, final int key, final V value) {
	 ...
	 */
	private UpdateResult<V> addToChain(final long bucket, final int key, final V value) {
		if (bucket == EMPTY) {
	 ...
	 * @return A new chain and an old value
	 */
	private UpdateResult<V> addToChainSlow(final long bucket, final Iterator<V> iter, final Block inputBlock, final int inputStartOffset, final int key, final V value) {
		...
		final Block outputBlock = m_blockAllocator.getThreadLocalBlock(chainLength + 2 * m_keySerializer.getMaxLength() + m_valueSerializer.getMaxLength(value) + 2); // 2
																																										// for
																																										// transition
																																										// from
																																										// header
		...
		while (true) {
			...
			if (res != null) {
				if (res.chain == bucket) // equal to 'bucket' - no changes
				{
					...
					final V ret = res.retValue; // must be saved in case of
												// rehash
					changeSize(res.sizeChange, buffers, getBlockLength(res.chain));
					return ret;
				}
	 ...
	 */
	private UpdateResult<V> removeKey(final long bucket, final int key) {
		final Block inputBlock = getBlockByIndex(bucket);
		if (inputBlock == null) {
		...
		final Block outputBlock = m_blockAllocator.getThreadLocalBlock(chainLength); // todo
																						// could
																						// be
																						// optimized,
																						// we
	...
	private static class Iterator<V> {
		...
		/**
		 * Memory allocation efficient method for looking up a value for a given
		 * key.
		 * 
		 * @param key
		 *            Key to look up
	...
	private static final class Writer<V> {
		...
		/**
		 * Optimization - copy the value binary representation ( do not try to
		 * (de)serialize it ). We always take the iterator key as a key.
		 * 
		 * @param iter
