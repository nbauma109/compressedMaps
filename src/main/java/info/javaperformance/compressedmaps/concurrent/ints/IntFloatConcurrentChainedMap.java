...
/**
 * A primitive concurrent hash map. It is built on top of a collection of memory
 * blocks managed by {@code ConcurrentBlockAllocator}. We use same serializers
 * as for non-concurrent collections here.
 *
 * This map does not use any locks, but it may spin using
 * {@code LockSupport.parkNanos} method in a few cases.
 *
 * {@code get} operation is implemented similarly to JDK
 * {@code ConcurrentHashMap} - it returns the value associated with a requested
 * key at the moment of method call (we get the bucket at the moment of method
 * call and extract a value out of it, while it can be updated in the mean time
 * by another thread.
 *
 * Both {@code put} and {@code remove} use CAS loops for updates, so they always
 * return the latest value (latest at the moment of successful update).
 *
 * Rehashing is done concurrently in this map. All 3 main operations
...
public class IntFloatConcurrentChainedMap implements IIntFloatConcurrentMap {
	private static final int CPU_NUMBER = Runtime.getRuntime().availableProcessors();
	private static final float NO_VALUE = 0;

	/*
	 * We store multiple reusable objects here. They are needed to avoid
	 * unnecessary object allocations. These objects should not be static - some
	 * of the are initialized with map specific serializers, some may simply
	...
	/** Value serializer */
	private final IFloatSerializer m_valueSerializer;
	/** Original fill factor */
	private final float m_fillFactor;
	/**
	 * We check if we have to rehash only after any bucket length exceeds this
	 * value. This allows us to avoid costly contended fields access.
	 */
	 ...
	 *            Expected map size
	 * @param fillFactor
	 *            Map fill factor. Fill factors over 1.0 are supported and
	 *            preferred for this map. This implementation puts a soft limit
	 *            of 16 for the fill factors. Such fill factors make buckets too
	 *            long, which increases the access/update costs, but the
	 *            difference in the compression is getting smaller and smaller.
	 *            Using fill factors below 1 is not prohibited, but you may end
	 *            up with worse memory consumption than
	 *            {@code ConcurrentHashMap} can provide you.
	 * @param keySerializer
	 *            Serializer for keys
	 ...
	 *             If {@code keySerializer == null} or
	 *             {@code valueSerializer == null}
	 * @throws IllegalArgumentException
	 *             If {@code fillFactor > 16} or {@code fillFactor <= 0.01}
	 */
	public IntFloatConcurrentChainedMap(final long size, final float fillFactor, final IIntSerializer keySerializer, final IFloatSerializer valueSerializer) {
		...
		// this threshold adjustment is needed on tiny initial size / large fill
		// factor combinations so that we do not
	...
	public float get(final int key) {
		...
		if (buffers.old != null) {
			rehash(buffers.nextStableVersion); // buffers.cur will be valid on
												// the exit from rehashing
		}
		...
		if (bucket == EMPTY) {
			// By definition of this method we return whatever value was valid
			// at a call time
			// We can get here either at a stable stage (old==null) or after
			// rehash, in both of which null bucket means no key.
			return NO_VALUE;
		}
		...
		if (input == null) {
			return get(key); // someone has relocated the data from this block,
								// need to retry because we have a valid bucket
		}
	...
	public float put(final int key, final float value) {
		...
		if (buffers.old != null) {
			rehash(buffers.nextStableVersion); // help rehashing, we are in
												// rehashing state already
		...
		while (true) {
			...
			/*
			 * Thread safety here: if we got to this point, we know that
			 * 'buffers' belong to stable state. It means it is either safe to
			 * set bucket here (stable state) or buffers.cur point to the old
			 * buffer (rehashing or subsequent states). During rehashing we
			 * atomically replace each cell with RELOCATED sentinel, which means
			 * 2 possibilities: 1) we replace current cell before it is picked
			 * up by rehashing thread (size could be safely increased here) 2)
			 * we try to CAS-replace a cell, but fail, because it was already
			 * replaced with RELOCATED (we go to 'else').
			 */
			if (res != null && compareAndSet(buffers.cur, idx, bucket, res.chain)) {
				...
				final float ret = res.retValue; // must be saved in case of
												// rehash
				changeSize(res.sizeChange, buffers, getBlockLength(res.chain));
				return ret;
			} else {
				// rollback output block usage. It is managed by a current
				...
				// cell could be changed to not empty / null (empty) in the
				// normal run
	...
	/*
	 * A version of {@code put}, which is called from rehashing only. It does
	 * not need to detect rehashing in progress.
	 */
	private void doPutRehash(final long[] tab, final int key, final float value) {
		final int idx = getIndex(key, tab.length);
		long bucket = getBucket(tab, idx);
		// We need to copy existing chain into a new buffer, add/update an
		// entry.
		// CAS in a loop
		while (true) {
			// copy/update the chain
	 ...
	 */
	private long singleEntry(final Block output, final int key, final float value) {
	 ...
	 */
	private UpdateResult addToChain(final long bucket, final int key, final float value) {
		if (bucket == EMPTY) {
		...
		if (iter.getElems() > MAX_ENCODED_LENGTH - 2) {
		...
		// 2* is a safety net here due to possibility that a value may take
		// longer in the delta form compared to original form
		...
		return getUpdateResult().set(pack(outputBlock.index, startOutputPos, iter.getElems() + sizeChange), retValue, sizeChange, inputBlock, outputBlock, startOutputPos);
	}
	 ...
	 * @param inputBlock
	 *            Input block
	 * @param inputStartOffset
	 *            Start offset for the iterator (prior to len)
	 * @param key
	 *            Key
	 * @param value
	 *            Value
	 * @return A new chain and an old value
	 */
	private UpdateResult addToChainSlow(final long bucket, final Iterator iter, final Block inputBlock, final int inputStartOffset, final int key, final float value) {
		boolean hasKey = false;
		float retValue = NO_VALUE;
		while (iter.hasNext()) // look up a key and then fast forward to the end
								// of chain to find out its length
		{
			iter.advance();
			if (iter.getKey() == key) {
				hasKey = true;
		...
		final Block outputBlock = m_blockAllocator.getThreadLocalBlock(chainLength + 2 * m_singleEntryLength + 2); // 2
																													// for
																													// transition
																													// from
																													// header
		...
		return getUpdateResult().set(pack(outputBlock.index, startOutputPos, MAX_ENCODED_LENGTH), retValue, hasKey ? 0 : 1, inputBlock, outputBlock, startOutputPos);
	}
	...
	public float remove(final int key) {
		...
		while (true) {
			...
			/*
			 * Possible returns: null - restart res.sizeChange = 0, res.chain =
			 * bucket - no key res.sizeChange = -1, res.chain = EMPTY - remove
			 * single key res.sizeChange = -1, res.chain != EMPTY - remove from
			 * long chain (output required)
			 */
			if (res != null) {
				...
				// CAS failed, rollback the output position, it is handled by
			...
			bucket = getBucket(buffers.cur, idx);
			if (bucket == RELOCATED) {
				rehash(buffers.nextStableVersion); // saved version is prior to
													// rehashing
				return remove(key);
	...
	private UpdateResult removeKey(final long bucket, final int key) {
		...
		outputBlock.pos = output.position();
		return getUpdateResult().set(pack(outputBlock.index, startOutputPos, iter.getElems() <= MAX_ENCODED_LENGTH ? iter.getElems() - 1 : MAX_ENCODED_LENGTH), retValue, -1,
				inputBlock, outputBlock, startOutputPos);
	}
	...
	private void rehash(final int nextStableVersion) {
		...
		if (buffers.resizeWorkers > 0) {
			// wait until all data is copied, all workers must finish before we
			// can proceed
	...
	private void rehashInnerLoop(final Buffers buffers) {
		...
		// start from random position and wrap round. It should reduce write
		// contention
		...
		for (int i = startPos; i < old.length; ++i) {
			if (!rehashInnerStep(old, dest, barLocal, iterLocal, i)) {
				i += old.length / CPU_NUMBER; // jump ahead to reduce contention
												// / quickly catch up
			}
		}
	...
	private boolean rehashInnerStep(final long[] old, final long[] dest, final ByteArray bar, final Iterator iter, final int idxOld) {
		...
		// Put RELOCATED into each processed cell. This way we distinguish
		// between not used and relocated cells.
		...
		if (bucket != RELOCATED && compareAndSet(old, idxOld, bucket, RELOCATED)) {
			...
			{
				iter.advance();
				// if CAS fails, the dest bucket is 1+ long, so we need to go
				// via a long path
	...
	private static class Iterator {
		...
		public Iterator(final IIntSerializer keySerializer, final IFloatSerializer valueSerializer) {
			m_keySerializer = keySerializer;
			m_valueSerializer = valueSerializer;
		}
		 ...
		 *            Byte buffer
		 * @param chainLength
		 *            Chain length stored in the header. 0xFF triggers reading
		 *            actual length from the bucket
		 ...
		 */
		public float findKey(final int key, final float noValue) {
			while (hasNext()) {
				advance();
				if (getKey() == key) {
					return getValue();
	...
	/**
	 * This class encapsulates the logic used to write all entries into the
	 * bucket.
	 */
	private static final class Writer {
		...
		public Writer(final IIntSerializer keySerializer, final IFloatSerializer valueSerializer) {
			m_keySerializer = keySerializer;
			m_valueSerializer = valueSerializer;
		}
		 ...
		 *            Underlying byte buffer
		 * @param elems
		 *            Number of elements to write, don't write anything if this
		 ...
		 *            Value to write
		 */
		public void writePair(final int k, final float v) {
			if (first) {
				m_keySerializer.write(k, buf);
				m_valueSerializer.write(v, buf);
				first = false;
			} else {
				// keys are sorted, so we can write unsigned diff (but
				// serializer will make a final decision)
	...
	/**
	 * All optimistic map changes are returned via this class instances. They
	 * contain enough information to commit/rollback these changes using CAS. An
	 * instance is always written and then read by the same thread, so no
	...
	private static class UpdateResult {
		...
		public UpdateResult set(long chain, float retValue, int sizeChange, Block input, Block output, int outputPrevStart) {
			this.chain = chain;
		...
		public String toString() {
			return "UpdateResult{" + "chain=" + chain + ", retValue=" + retValue + ", sizeChange=" + sizeChange + ", input=" + input + ", output=" + output + ", outputPrevStart="
					+ outputPrevStart + '}';
		}
	}
	...
	private void changeSize(final int delta, final Buffers curBuffers, final int bucketLength) {
		...
		if (bucketLength > m_iFillFactor && calculateSize() > curBuffers.threshold) {
			...
			// check if we got too close to the max array size, otherwise we
			// will have to make a pretty useless resize when got close to
			// 2G*fillFactor entries
			if (newCapacity * 1.5 > Primes.getMaxIntPrime()) {
				newCapacity = Primes.getMaxIntPrime();
			}
			// this check disables rehashing after a table has reached the
			// maximal size
			final long newThreshold = newCapacity >= Primes.getMaxIntPrime() ? Long.MAX_VALUE : curBuffers.threshold * multiplier;
			// Entering rehashing mode. It does not matter which thread changes
			// it, we still have to enter {@code rehash}.
			// Using if-else here helps with debugging
			if (m_data.compareAndSet(curBuffers, new Buffers(m_longAlloc.allocate(newCapacity), curBuffers.cur, newThreshold, curBuffers.version + 1, // switching
																																						// to
																																						// rehashing
																																						// mode
					curBuffers.nextStableVersion // next stable version does not
													// change at this moment
			))) {
				rehash(curBuffers.nextStableVersion); // we must use cached
														// version here, actual
														// may be greater
			} else {
				rehash(curBuffers.nextStableVersion); // we must use cached
