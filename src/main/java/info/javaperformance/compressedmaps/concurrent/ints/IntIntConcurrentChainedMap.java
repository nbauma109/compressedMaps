...
public class IntIntConcurrentChainedMap implements IIntIntConcurrentMap {
	 ...
	 */
	public IntIntConcurrentChainedMap(final long size, final float fillFactor, final IIntSerializer keySerializer, final IIntSerializer valueSerializer) {
	 ...
	 */
	private void doPutRehash(final long[] tab, final int key, final int value) {
		final int idx = getIndex(key, tab.length);
	 ...
	 */
	private long singleEntry(final Block output, final int key, final int value) {
	 ...
	 */
	private UpdateResult addToChain(final long bucket, final int key, final int value) {
		if (bucket == EMPTY) {
	 ...
	 * @return A new chain and an old value
	 */
	private UpdateResult addToChainSlow(final long bucket, final Iterator iter, final Block inputBlock, final int inputStartOffset, final int key, final int value) {
		...
		while (true) {
			...
			if (res != null) {
				if (res.chain == bucket) // equal to 'bucket' - no changes
				{
					...
					final int ret = res.retValue; // must be saved in case of
													// rehash
					changeSize(res.sizeChange, buffers, getBlockLength(res.chain));
					return ret;
				}
	...
	private static class Iterator {
		 ...
		 */
		public int findKey(final int key, final int noValue) {
			while (hasNext()) {
				advance();
				if (getKey() == key) {
					return getValue();
